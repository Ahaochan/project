...
# 4. 配置文件
mkdir -p /etc/sentinel /var/sentinel/${PORT} /var/log/sentinel/${PORT}
...
sed -i "s#^sentinel down-after-milliseconds mymaster [[:digit:]]*#sentinel down-after-milliseconds mymaster 30001#g" /etc/sentinel/${PORT}.conf
...
sed -i "s#^logfile .*#/var/log/sentinel/${PORT}#g" /etc/sentinel/${PORT}.conf
...
# 7. 删除产生的文件
rm -rf /var/sentinel/${PORT}.conf /var/sentinel/${PORT} /var/run/sentinel_${PORT}.pid
