...
import static moe.ahao.tend.consistency.core.utils.DefaultValueUtils.getOrDefault;
...
public class TendConsistencyAutoConfiguration {
    ...
    private void doConfigCheck(TendConsistencyParallelTaskConfigProperties consistencyParallelTaskConfigProperties,
        ...
        if (StringUtils.isNotEmpty(consistencyParallelTaskConfigProperties.getThreadPoolKeepAliveTimeUnit())) {
            try {
        ...
        if (StringUtils.isNotEmpty(consistencyParallelTaskConfigProperties.getTaskScheduleTimeRangeClassName())) {
            // 校验是否存在该类
            ...
            if (taskScheduleTimeRangeClass == null) {
    ...
    private Long getTaskShardingCountOrDefault(TaskExecuteEngineConfigProperties properties) {
        if (properties.getTaskShardingCount() != null) {
    ...
    private boolean peerIdIsDuplicate() {
        ...
        for (String peer : peersConfigList) {
            ...
            if (!NumberHelper.isNumber(peerId)) {
