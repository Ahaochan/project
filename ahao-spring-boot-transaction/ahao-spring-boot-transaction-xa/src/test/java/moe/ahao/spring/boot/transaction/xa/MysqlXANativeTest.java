...
public class MysqlXANativeTest {
    ...
    public void mysql() throws Exception {
        ...
        connection.createStatement().execute("create database if not exists ahaodb");
        connection.createStatement().execute("drop table if exists ahaodb.user");
        connection.createStatement().execute("create table ahaodb.user ( id int primary key, username varchar(50))");
        connection.createStatement().execute("insert ahaodb.user (id, username) values (1, 'user1'), (2, 'user2')");
        connection.close();

        // 这个XAResource其实你可以认为是RM（Resource Manager）的一个代码中的对象实例
        Connection connection1 = DriverManager.getConnection(url + "/ahaodb", "root", "root");
        XAConnection xaConnection1 = new MysqlXAConnection((JdbcConnection) connection1, true);
        XAResource xaResource1 = xaConnection1.getXAResource();

        // 这个XAResource其实你可以认为是RM（Resource Manager）的一个代码中的对象实例
        Connection connection2 = DriverManager.getConnection(url + "/ahaodb", "root", "root");
        XAConnection xaConnection2 = new MysqlXAConnection((JdbcConnection) connection2, true);
