...
public class InTransferServiceImpl implements InTransferService {
    ...
    public void prepare(BusinessActionContext actionContext, String json) throws Exception {
        ...
        // 标识try阶段开始执行
        logger.info("prepare阶段标记start, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);
        ...
        if (this.isEmptyRollback()) {
            logger.info("prepare阶段出现悬挂, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);
            throw new IllegalStateException("出现悬挂");
        }

        logger.info("prepare阶段进行, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);
        bankTransferAccountMybatisService.increasePrepare(accountId, amount);

        // 标识try阶段执行成功
        logger.info("prepare阶段标记success, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);
    ...
    public void confirm(BusinessActionContext actionContext) throws Exception {
        ...
        Long accountId = transferDTO.getAccountId();
        BigDecimal amount = transferDTO.getAmount();
        logger.info("confirm阶段开始, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);

        // 当出现网络异常或者TC Server异常时, 会出现重复调用commit阶段的情况, 所以需要进行幂等判断
        if (!TccResultHolder.isTrySuccess(getClass(), transferNo, xid)) {
            logger.info("confirm阶段幂等性校验失败, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);
            return;
        }

        logger.info("confirm阶段进行, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);
        bankTransferAccountMybatisService.increaseConfirm(accountId, amount);

        //移除标识
        logger.info("confirm阶段结束, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);
    ...
    public void cancel(BusinessActionContext actionContext) throws Exception {
        ...
        Long accountId = transferDTO.getAccountId();
        BigDecimal amount = transferDTO.getAmount();
        logger.info("cancel阶段开始, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);

        // 空回滚处理
        if (TccResultHolder.isTagNull(getClass(), transferNo, xid)) {
            logger.info("cancel阶段发生空回滚, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);
        ...
        if (!TccResultHolder.isTrySuccess(getClass(), transferNo, xid)) {
            logger.info("cancel阶段幂等性校验失败, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);
            return;
        }

        logger.info("cancel阶段进行, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);
        bankTransferAccountMybatisService.increaseCancel(accountId, amount);

        // 移除标识
        logger.info("cancel阶段结束, 入账, 单号:{}, accountId:{}, amount:{}, xid:{}", transferNo, accountId, amount, xid);
        TccResultHolder.removeResult(getClass(), transferNo, xid);
    }
