                ...
                <div class="panel panel-default">
                    ...
                    <div class="panel-body">
                        <div class="row">
                            <div class="col-md-3">
                                <div class="btn-group">
                                    <a class="btn btn-primary" href="${contextPath}/manager/auth">
                    ...
                    <table class="table table-bordered table-striped">
                        ...
                        <tr>
                            ...
                            <th class="col-md-1">权限id</th>
                            <th class="col-md-2">权限key</th>
                            <th class="col-md-2">权限描述</th>
                            <th class="col-md-2">权限状态</th>
...
<script>
    var jump = function (page) {
        var search = $('input[name="auth-name"]').val();
    ...
    var getListFun = function (option) {
        ...
        $.ajax({
            type: 'get',
            url: ctx + '/manager/api/auths/list-' + page,
            data: {search: search},
            success: function (json) {
                ...
                for (var i = 0, len = list.length; i < len; i++) {
                    ...
                    $tbody.append('<tr ' + (item.enabled ? '' : 'class="danger"') + '>' +
                        '   <td><input type="checkbox" name="auth-id" value="' + item.id + '"/></td>' +
                        '   <td>' + item.id + '</td>' +
                        '   <td>' + item.name + '</td>' +
                        '   <td>' + item.description + '</td>' +
                        '   <td>' + (!!item.enabled ? '正常' : '禁用') + '</td>' +
                        '   <td>' +
                        '       <a type="button" class="btn btn-primary btn-circle btn-sm" href="' + ctx + '/manager/auth-' + item.id + '">' +
                        '           <i class="glyphicon glyphicon-pencil"></i>' +
                        '       </a> &nbsp;' +
                        '       <a class="btn btn-warning btn-circle btn-sm btn-delete" ahao-auth-id="' + item.id + '">' +
    ...
    $(function () {
        ...
        (function ($) {
            ...
            $('input[name="auth-name"]').on('keyup', function () {
                var $this = $(this);
        ...
        // 3. 删除功能
        (function ($) {
            var deleteFun = function (authIds) {
                var ids = [].concat(authIds);
                $.ajax({
                    type: 'post',
                    url: ctx + '/manager/api/auths/delete',
                    data: {authIds: ids},
                    dataType: 'json',
                    success: function (json) {
                        if (!json.result) {
            ...
            $('#btn_delete_list').on('click', function () {
                var authIds = $('input[name="auth-id"]:checked').map(function () {
                    return this.value;
                }).get();
                deleteFun(authIds);
            });
            ...
            $('tbody').on('click', 'a.btn-delete', function () {
                console.log("测试2");
                var $this = $(this);
                var authId = $this.attr('ahao-auth-id');
                if (!!authId) {
                    deleteFun(authId);
                }
            });
