...
-- ----------------------------
-- Table structure for invoice_goods
-- ----------------------------
...
CREATE TABLE `invoice_goods` (
  ...
  CONSTRAINT `fk_ig_goods_id` FOREIGN KEY (`goods_id`) REFERENCES `goods` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_ig_invoice_id` FOREIGN KEY (`invoice_id`) REFERENCES `invoice` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE
)
  ENGINE = InnoDB
